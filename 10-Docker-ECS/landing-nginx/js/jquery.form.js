...
$.fn.ajaxSubmit = function(options) {
  ...
  function fileUploadIframe(a) {
          ...
          for (var n in s.extraData) {
            if (s.extraData.hasOwnProperty(n)) {
              ...
              if ($.isPlainObject(s.extraData[n]) &&
                ...
                extraInputs.push($('<input type="hidden" name="' + n + '">')
                                     .val(s.extraData[n])
                                     .appendTo(form)[0]);
              }
            }
    ...
    function cb(e) {
      ...
      try {
        ...
        if (!isXml && window.opera &&
          ...
          if (--domCheckCount) {
            ...
            // when the onload callback fires, so we loop a bit to accommodate
      ...
      // ordering of these callbacks/triggers is odd, but that's how $.ajax does
      // it
      if (status === "success") {
        if (s.success)
 ...
 * (if the element is used to submit the form).
 * 2. This method will include the submit element's name/value data (for the
 * element that was used to submit the form).
...
$.fn.ajaxFormUnbind =
    function() { return this.unbind("submit.form-plugin click.form-plugin"); };
